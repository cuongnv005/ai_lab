...
Chuẩn: Testing Library, MSW node, renderWithProviders, TC-ID, tiếng Việt.
...
CHỈ được tạo:
  ...
  __tests__/mocks/handlers/*.handlers.ts
  __tests__/coverage-maps/*.md
  __tests__/fixtures/*.tsx
```
...
| Coverage map đã có? | `cat __tests__/coverage-maps/<domain>.<feature>.md` |
| MSW handlers đã có? | `ls __tests__/mocks/handlers/` |
...
### 0e. Đọc MSW handlers hiện có

```bash
ls __tests__/mocks/handlers/ 2>/dev/null
```

- Đã có → import lại, không duplicate
- Chưa có → ghi danh sách cần tạo mới
...
### Cấu trúc (CHỈ trong `__tests__/`)
```
__tests__/
├── setup.ts
├── fixtures/auth.fixture.tsx        ← renderWithProviders
├── mocks/
│   ├── server.ts
│   └── handlers/<feature>.handlers.ts
├── coverage-maps/<domain>.<feature>.md
└── tests/<domain>/<feature>.test.tsx   ← FILE TẠO MỚI
```

> ⛔ **TUYỆT ĐỐI KHÔNG** tạo `e2e/` files.
> ⛔ **TUYỆT ĐỐI KHÔNG** tạo `*.spec.ts` (bất kỳ đâu).
> ✅ **CHỈ** tạo `*.test.tsx`, `*.handlers.ts`, `*.fixture.tsx`, `coverage-maps/*.md`.
...
import { renderWithProviders } from '../../fixtures/auth.fixture';
...
```typescript
// __tests__/fixtures/auth.fixture.tsx
import { ReactElement } from 'react';
import { render, RenderOptions } from '@testing-library/react';
import { QueryClient, QueryClientProvider } from '@tanstack/react-query';
import { NextIntlClientProvider } from 'next-intl';
import vi_messages from '@/i18n/messages/vi.json';
...
export function renderWithProviders(
  ui: ReactElement,
  { locale = 'vi', messages = vi_messages, queryClient, ...opts }: Options = {}
) {
...
### Template MSW handler
```typescript
// __tests__/mocks/handlers/<feature>.handlers.ts
import { http, HttpResponse } from 'msw';

export const <feature>Handlers = [
  http.post('/api/<endpoint>', () =>
    HttpResponse.json({ /* success payload */ }, { status: 200 })
  ),
];
```
...
## Bước 5 — Tạo/Update Coverage Map

File: `__tests__/coverage-maps/<domain>.<feature>.md`

```markdown
# Coverage Map — <domain>/<feature>
<!-- Tạo bởi create-tc-component — ĐỪNG SỬA TAY -->
<!-- Cập nhật bởi create-tc-flow khi fill PW-XX -->

## Vitest (Component/Logic) — claimed by create-tc-component
| TC-ID | Scenario | Status |
|-------|----------|--------|
| VT-01 | Render đúng elements | ✅ written |
| VT-02 | Submit → action gọi đúng args | ✅ written |
| VT-03 | Loading state — button disabled | ✅ written |
| VT-04 | Validation — submit rỗng | ✅ written |
| VT-05 | Validation — thiếu field A | ✅ written |
| VT-06 | Validation — thiếu field B | ✅ written |
| VT-07 | Format sai → error Zod | ✅ written |
| VT-08 | Store error → alert | ✅ written |
| VT-09 | Toggle password type | ✅ written |
| VT-10 | Re-type → clear error | ✅ written |
| VT-11 | MSW 400 → API error (component) | ✅ written |
| VT-12 | MSW 500 → generic error (component) | ✅ written |

## Playwright (Flow/URL/Cookie) — to be claimed by create-tc-flow
| TC-ID | Scenario | Status |
|-------|----------|--------|
| PW-01 | Redirect URL sau submit thành công | ⏳ pending |
| PW-02 | Enter key → redirect | ⏳ pending |
| PW-03 | Auth guard — đã login → redirect home | ⏳ pending |
| PW-04 | Auth guard — chưa login → redirect /login | ⏳ pending |
| PW-05 | Cookie/token set sau login | ⏳ pending |
| PW-06 | API 500 real intercept → page error | ⏳ pending |
| PW-07 | Token hết hạn → redirect /login | ⏳ pending |
| PW-08 | Logout → redirect + cookie xóa | ⏳ pending |
| PW-09 | Double submit real timing | ⏳ pending |
| PW-10 | Back button sau login | ⏳ pending |

## Uncovered (gap)
- [ ] <scenario nếu không thuộc cả 2 domain>

## Combined Coverage Estimate
- Vitest: ~90% component logic
- Playwright: ⏳ TBD (chạy create-tc-flow)
- Total: ⏳ TBD
```

> **Append rule:** File đã tồn tại → chỉ ADD row VT-XX/VT-DS-XX mới, KHÔNG xóa row cũ.
...
it('VT-DS-07: Status badge dùng variant semantic, không hardcode color', () => {
  renderWithProviders(<FeatureComponent status="active" />);
...
✅ Đã tạo (Vitest only):
  - __tests__/tests/<domain>/<feature>.test.tsx
  - __tests__/mocks/handlers/<feature>.handlers.ts  (nếu mới)
  - __tests__/coverage-maps/<domain>.<feature>.md
