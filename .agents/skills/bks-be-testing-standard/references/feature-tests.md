...
Before writing test cases, analyze the source code:
1. **Read the `FormRequest`'s `rules()` method** and identify **EVERY** rule type across all fields.
2. **Check for custom rules** and analyze their internal logic for success/failure paths.
3. **Read `Service` and `Controller` logic** to identify all logic branches (`if`, `ternary`, `match`, `switch`).
4. Apply these techniques when designing test cases and selecting test data to ensure maximum coverage with minimum redundancy.
