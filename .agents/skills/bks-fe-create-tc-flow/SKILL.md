...
Chuẩn: Page Object Model, fixture, TC-ID, tiếng Việt.
...
CHỈ được tạo:
  e2e/pages/*.page.ts
  e2e/tests/**/*.spec.ts
  __tests__/coverage-maps/*.md  (chỉ update dòng PW-XX)
```
...
> **Bắt buộc:**
> 1. Đọc coverage-map nếu tồn tại → biết VT-XX đã claim, PW-XX nào còn `⏳ pending`
> 2. Đọc Vitest file tương ứng → xác nhận KHÔNG viết lại TC đã có
> 3. Nếu chưa có cả 2 → tạo coverage-map mới, bắt đầu từ PW-01
...
├── fixtures/auth.fixture.ts    ← giữ nguyên nếu đã có
...
## Bước 5 — Update Coverage Map

File: `__tests__/coverage-maps/<domain>.<feature>.md`

**Nếu file tồn tại (create-tc-component đã chạy):**
1. Update PW-XX từ `⏳ pending` → `✅ written`
2. Tính Combined Coverage Estimate

**Nếu file KHÔNG tồn tại:**
```markdown
# Coverage Map — <domain>/<feature>
<!-- Tạo bởi create-tc-flow — ĐỪNG SỬA TAY -->

## Vitest (Component/Logic) — to be claimed by create-tc-component
| TC-ID | Scenario | Status |
|-------|----------|--------|
| VT-01 | Render đúng elements | ⏳ pending |
| VT-02 | Submit → action gọi đúng args | ⏳ pending |
| VT-03 | Loading state — button disabled | ⏳ pending |
| VT-04 | Validation — submit rỗng | ⏳ pending |
| VT-05 | Validation — thiếu field A | ⏳ pending |
| VT-06 | Validation — thiếu field B | ⏳ pending |
| VT-07 | Format sai → error Zod | ⏳ pending |
| VT-08 | Store error → alert | ⏳ pending |
| VT-09 | Toggle password type | ⏳ pending |
| VT-10 | Re-type → clear error | ⏳ pending |

## Playwright (Flow/URL/Cookie) — claimed by create-tc-flow
| TC-ID | Scenario | Status |
|-------|----------|--------|
| PW-01 | Happy path → redirect đúng URL | ✅ written |
| PW-02 | Enter key → redirect | ✅ written |
| PW-03 | Cookie/token set sau login | ✅ written |
| PW-04 | Auth guard: đã login → redirect home | ✅ written |
| PW-05 | Auth guard: chưa login → redirect /login | ✅ written |
| PW-06 | Token hết hạn → redirect /login | ✅ written |
| PW-07 | API 500 → error, không redirect | ✅ written |
| PW-08 | API 400 → error, không redirect | ✅ written |
| PW-09 | Logout → redirect + cookie xóa | ✅ written |
| PW-10 | Back button không về /login | ✅ written |

## Uncovered (gap)
- [ ] <nếu có>

## Combined Coverage Estimate
- Vitest: ⏳ pending (chạy create-tc-component)
- Playwright: ~95% E2E critical paths
- Total: ⏳ TBD
```

> **Append rule:** File đã tồn tại → chỉ ADD row PW-XX mới, KHÔNG xóa row cũ.
...
✅ Đã tạo:
  ...
  - __tests__/coverage-maps/<domain>.<feature>.md  ← UPDATED
