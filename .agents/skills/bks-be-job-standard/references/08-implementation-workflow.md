...
Implement logic in `app/Services/Background/{Module}/`. The `run()` method MUST accept the DTO. Ensure **Manual Transactions** (for multi-query/locking) and N+1 prevention.
...
```php
// tests/Unit/{Module}/{Feature}BackgroundServiceTest.php
use Tests\TestCase;
use Illuminate\Foundation\Testing\RefreshDatabase;
use App\Services\Background\{Module}\{Feature}BackgroundService;
