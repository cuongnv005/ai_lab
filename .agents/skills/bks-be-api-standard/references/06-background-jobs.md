...
2. **Background Service**: `app/Services/Background/{Module}/{Feature}BackgroundService.php` with `run({Action}Data $dto)`
3. **Register** in `BackgroundFactory` (add getter method — never create a new factory)
4. **Job stub**: `app/Jobs/{Module}/{Feature}Job.php` — passes DTO to Background Service
