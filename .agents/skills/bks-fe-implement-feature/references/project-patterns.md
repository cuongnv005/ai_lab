...
- **Zod Response Validation (Runtime Guard):** To secure the front-end boundaries, we enforce runtime validation on the `data` field of the HTTP responses in our repositories.
  ```ts
  ...
  // In http-<feature>.repository.ts
  const res = await HttpService.get<unknown, ResponseData<User>>(`admin/users/${id}`)
  // Parse response data safely before returning
  res.data.data = UserResponseSchema.parse(res.data.data)
  return res
  ```
...
Every feature must use `useTranslation()` for all user-visible text. Hardcoded strings are forbidden.

### Key Structure
- **`action.*`**: Shared actions (Save, Cancel, Search, Create, etc.).
- **`validation.*`**: Shared validation templates (Required, Min, Max, etc.).
- **`<feature>.*`**: Feature-specific text (Labels, Titles, Messages).

**Strict Rules:**
- **No Redundancy:** Check shared roots BEFORE adding feature keys. Use existing keys if available.
- **Perfect Sync:** Every key MUST exist in both `vi.json` and `en.json`.
- **Zero Omissions:** Every user-visible string must have a key.
...
- See `validation-i18n.md` for the `validation.*`, `action.*` vs `<feature>.*` contract.
...
- **No tuple returns** like `const [data, setData] = useFeature()` for non-trivial hooks. Tuples
