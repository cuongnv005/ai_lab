...
Implement logic in `app/Services/Background/{Module}/`. The `run()` method MUST accept the DTO as its parameter. Apply **Manual Transactions** (for multi-query/locking) and N+1 prevention.
