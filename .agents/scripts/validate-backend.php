...
class ApiStructureValidator
{
    ...
    private function validateServices(string $projectPath): void
    {
        ...
        foreach ($services as $service) {
            ...
            if (!$isBaseClass && !$isCommonService && !$isTableService && !preg_match('/[A-Z][a-zA-Z]+Data\s+\$/', $content)) {
...
class DatabaseStructureValidator
{
    ...
    private function validateMigrations(string $projectPath): void
    {
        ...
        foreach ($migrations as $migration) {
            ...
            if (preg_match('/\$table->timestamps/', $content)) {
                // Good - timestamps are present
            } else {
                if (!preg_match('/create\([\'"]\w+_\w+[\'"]/', $content)) {
    ...
    private function validateModels(string $projectPath): void
    {
        ...
        foreach ($models as $model) {
            ...
            if (preg_match('/protected \$casts/', $content) && !preg_match('/AsEnumCollection|AsStringable/', $content)) {
...
class JobStructureValidator
{
    ...
    public function validate(string $projectPath): array
    {
        ...
        if (!is_dir($jobPath)) {
            return ['valid' => true, 'errors' => [], 'warnings' => ["Jobs directory not found at: $jobPath"], 'total' => 0];
        }
...
class MasterDataStructureValidator
{
    ...
    public function validate(string $projectPath): array
    {
        ...
        if (!is_dir($driverPath)) {
            return ['valid' => true, 'errors' => [], 'warnings' => ["MasterData driver directory not found"], 'total' => 0];
        }
