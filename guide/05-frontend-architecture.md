...
│   └── [locale]/           # Tất cả route được bọc trong locale (next-intl)
...
│   └── mocks/              # MSW setup (browser.ts, server.ts)
│
...
```text
middleware.ts
  ├── Bước 1: next-intl — xử lý locale redirect (VD: /login → /vi/login)
...
Toàn bộ cây Provider được khai báo trong `app/[locale]/providers.tsx` theo thứ tự lồng nhau có chủ đích:

```text
<ThemeProvider>           ← Dark/Light mode
  <QueryClientProvider>   ← TanStack Query client
    <MSWProvider>         ← Mock Service Worker (chỉ active khi NEXT_PUBLIC_API_MOCK=true)
...
*   Tạo Page trong `app/[locale]/(main)/{module}/page.tsx`.
