...
│   ├── setup.ts
│   ├── fixtures/
│   │   └── auth.fixture.tsx          # renderWithProviders (QueryClient + NextIntl)
│   ├── mocks/
│   │   ├── server.ts                 # MSW node server
│   │   └── handlers/
│   │       └── <feature>.handlers.ts # MSW handlers per feature
│   ├── coverage-maps/
│   │   └── <domain>.<feature>.md     # Bảng TC coverage (VT + PW)
│   └── tests/
│       └── <domain>/
...
└── e2e/
    ...
    ├── fixtures/
    │   └── auth.fixture.ts           # Playwright auth helper
    └── tests/
        └── <domain>/
...
Mỗi feature phải có một file `__tests__/coverage-maps/<domain>.<feature>.md` theo mẫu:

```markdown
# Coverage Map — <domain>/<feature>
<!-- Tạo bởi create-tc-component / create-tc-flow — ĐỪNG SỬA TAY -->

## Vitest (Component/Logic) — claimed by create-tc-component
| TC-ID | Scenario | Status |
|-------|----------|--------|
| VT-01 | Render đúng elements | ✅ written |
| VT-DS-01 | Error text dùng text-destructive | ✅ written |
...
- Vitest: ~90% statements
- Playwright: ~95% critical E2E paths
- Total: ✅ ~100% critical paths covered
```

> [!NOTE]
> File coverage-map là điểm kết nối duy nhất giữa hai tầng. Không được sửa tay — luôn để skill cập nhật tự động.
...
### 13.2. Không dùng `renderWithProviders`

```typescript
// ❌ Sai — Component dùng TanStack Query + next-intl nhưng không có Provider
render(<LoginForm />); // ❌ bị lỗi context

// ✅ Đúng — Luôn dùng renderWithProviders từ auth.fixture.tsx
renderWithProviders(<LoginForm />);
```
